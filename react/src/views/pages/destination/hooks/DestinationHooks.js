...
const DestiantionHooks = (props) =>{
    ...
    const [destionationDetails, setDestionationDetails] = useState(false);
    const handleOpen = ()  => setOpen(true);
    const handleClose = () => {
        setOpen(false)
        setDestionationDetails({});
    };

    const handleModal= (data) => {
        handleOpen();
        setDestionationDetails(data);
        setSearchParams({})
    }
    ...
    const submit = async (values) => {
        ...
        try {
            ...
            let res;
            if (id) {
                res = await putData(`destination/${id}`, data);
            } else {
                res = await postData('/destination', data);
            }
            await dispatch({
